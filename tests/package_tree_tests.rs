...
use std::path::Path;
use stern4rust::package_tree::PackageTree;
...
#[test]
fn directories_lists_every_directory_a_file_sits_in() {
    // Arrange
    let tree = tree(&["src/lib.rs", "src/rules/mod.rs", "tests/all_tests.rs"]);
    ...
    assert_eq!(
        directories,
        [Path::new("src"), Path::new("src/rules"), Path::new("tests")]
    );
}
